...
public class Client1 {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        client.connect(new InetSocketAddress(hostname, port), null, new HandlerC1(client, configuration));

        Thread.currentThread().join();
    }
}

class HandlerC1 implements CompletionHandler<Void, Void> {
    private final AsynchronousSocketChannel client;
    private final Configuration configuration;

    public HandlerC1(AsynchronousSocketChannel client, Configuration configuration) {
        this.client = client;
        ...
        if (client.isOpen()) {
            ...
            client.read(receiveBuffer, null, new CompletionHandler<Integer, Void>() {
                ...
                public void completed(Integer bytesRead, Void attachment) {
                    ...
                    System.out.println("Client received the value: " + receivedNumber);

                    FunctionF function = new FunctionF(receivedNumber, configuration);

                    int[] result = function.f();
...
class FunctionF {
    ...
    public FunctionF(int x, Configuration configuration) {
        this.x = x;
        this.timeLimit = configuration.getTimeLimit();
    }
